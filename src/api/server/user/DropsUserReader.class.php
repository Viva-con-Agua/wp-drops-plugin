    ...
    public function run()
    {
        ...
        if (empty($user)) {
			...
			return (new DropsResponse())
				->setCode(400)
				->setContext(__CLASS__)
				->setMessage('No user with given id! Parameters: ' . implode(', ', $this->userData));

        }
            
		$mail_switch = $this->dataHandler->getUserMetaById($userId);
		$mail_switch = empty($mail_switch) ? 'none' : $mail_switch;
